...
export default function NotifPanel({ open, onClose, claims, policies = [] }) {
  const notifs = [];
  claims.forEach((c) => {
    const s = slaInfo(c);
  ...
  policies.forEach((p) => {
    notifs.push({ id: p._id + "-policy", icon: "policy", title: `New policy assigned: ${p.policyId}`, body: `${p.insurer} — ${p.category}`, time: "New" });
  });
  if (!open) return null;
  return (
        ...
        <div className="divide-y divide-ink-900/6">
          ...
          {notifs.map((n) => (
            <div key={n.id} className="px-4 py-3 flex gap-3 hover:bg-navy-50/50">
              <span className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${n.icon === "alert" ? "bg-red-100 text-red-600" : n.icon === "warn" ? "bg-amber-100 text-amber-600" : n.icon === "policy" ? "bg-navy-100 text-navy-700" : "bg-emerald-100 text-emerald-600"}`}>
                {n.icon === "ok" ? <Check className="w-4 h-4" /> : n.icon === "policy" ? <Hash className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
              </span>
